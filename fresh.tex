...
\begin{abstract}
Storage systems serve as the backbone of today's data-intensive applications, such as financial trading and large-scale database systems. In these applications, storage behavior directly impacts user experience. However, the complexity of modern workloads makes it difficult to identify and analyze performance and wear-leveling issues. While visualization is essential for such analysis, existing SSD simulators rarely provide this capability, hindering in-depth research.
To address this, we introduce \eyanassdsim, a web-based simulator that visualizes internal operations and data placement for intuitive performance and wear-leveling analysis.

\eyanassdsim provides real-time visual tracking of internal SSD operations, including data placement, page invalidation, garbage collection, and block erasure. The simulator enables performance analysis through quantitative metrics such as write amplification factor (WAF), wear-leveling distribution, and workload access pattern analysis. We validate \eyanassdsim against established simulators (FEMU, FTLSim), demonstrating consistent results. As an accessible, installation-free tool, \eyanassdsim bridges the gap between SSD complexity and user comprehension for both research and education.
...
In the remainder of this paper, we introduce background and related work in Section~\ref{subsec:related_work}.
...
Most storage monitoring tools display throughput and latency over time. However, they rarely expose internal SSD behavior such as block-level access patterns or garbage collection activity. Among SSD simulators, \mqsim~\cite{MQSim-USENIX18} supports multiple queue models and provides performance metrics but lacks visual feedback on internal data placement.
\flashsim~\cite{FlashSim-FICASS-09} offers page-level simulation of flash operations yet does not provide interactive visualization. \ssdmodel~\cite{SSDModel-USENIX-TC-08} and similarly structured simulators often require significant setup time and technical expertise for installation, limiting their accessibility. \nandflashsim~\cite{NANDFlashSim-TOS-16} focuses on detailed NAND-level modeling, while \vssim~\cite{VSSIM-MSST-13} and \wiscsim~\cite{WiscSim-EuroSys-17} target performance evaluation without visualization support. \ssdplayer~\cite{visualizationSSDPlayer} offers block-level data placement visualization but is limited in workload flexibility, real-time interactivity, and web-based accessibility. \simplessd~\cite{SimpleSSD-IEEE18} offers high-fidelity simulation integrated with gem5, prioritizing accuracy over accessibility.

Table~\ref{table:features} summarizes the key features of these simulators alongside \eyanassdsim, highlighting gaps in the literature --- particularly the absence of real-time visualization and web-based accessibility. \eyanassdsim addresses these gaps by prioritizing analytical, educational, and research-oriented design, offering on-demand web access, real-time monitoring, and comprehensive documentation without requiring installation. Notably, real-time visualization has proven valuable in other system domains such as IoT-based monitoring~\cite{irrigation-wsn-ieee}, underscoring the broader importance of intuitive visual feedback for system analysis and comprehension.
...
In this section, the simulation results are analyzed through mathematical formulations. A summary of the notations used in Equations~(\ref{eq:doipd}), (\ref{eq:doec}), and (\ref{eq:degree_of_amplitude}) is provided in Table~\ref{table:math_notations}.

\begin{table}[h]
 \centering
 \caption{Summary of the mathematical notations used}
...
Fig.~\ref{fig:erasecount} presents a histogram with bell curves to analyze the erase counts for SSD blocks under three access patterns: sequential, uniform random, and Zipf workloads. The sequential workload exhibits the lowest mean erase count (\(\mu = 1.65\)) and the smallest DoEC (\(DoEC = 2.92\)), indicating more uniform wear across the SSD blocks. In contrast, the uniform random workload shows higher mean erase counts (\(\mu = 3.02\)) and increased variability (\(DoEC = 4.38\)), while the Zipf workload displays the highest variability (\(DoEC = 4.93\)), suggesting highly uneven wear. Consequently, the sequential workload demonstrates the most favorable performance for wear-leveling.
...
\begin{figure}
 ...
 \caption{Bar chart comparing standard deviation of Fourier Transform amplitudes ($\sigma_A$) across five allocation policies (S1--S5) for sequential, uniform random, and Zipf workloads. Lower $\sigma_A$ values indicate more uniform erase count distribution and better wear-leveling performance.}
